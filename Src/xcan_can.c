...
{
  xcan_can_rx_frame( hcan, CAN_RX_FIFO0 );
}

void HAL_CAN_RxFifo1MsgPendingCallback( CAN_HandleTypeDef *hcan )
{
  xcan_can_rx_frame( hcan, CAN_RX_FIFO1 );
}
...
void HAL_CAN_ErrorCallback( CAN_HandleTypeDef *hcan )
{
  ...
  if( can_dev.err_cb && can_err )
  {
    can_dev.err_cb( can_err, can_dev.tx_errs & 0xFF, can_dev.rx_errs );
  }
